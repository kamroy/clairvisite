...
export const REPORT_ALREADY_SUBMITTED_MESSAGE = 'Ce rapport a déjà été soumis et ne peut plus être modifié.';

@Injectable()
export class UpdateReportConclusionUseCase {
  constructor(
  ...
  async execute(technicianUserId: string, bookingId: string, generalConclusion: string) {
    ...
    return this.reports.updateConclusion(report.id, generalConclusion);
  }
}
